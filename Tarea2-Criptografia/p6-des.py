...
# (Por brevedad visual en este bloque, aqui van las S_BOXES exactamente como las tienes en tu README.md)
# Asegurate de pegar la matriz S_BOXES original del profesor aqui.
S_BOXES = [
...
def main():
    ...
    message_text = "noche697"
    key_text = "data7Qa="
    
    # 2. Convertir a bytes para el cifrado (el core espera int.from_bytes)
    message_bytes = message_text.encode("utf-8")
    key_bytes = key_text.encode("utf-8")
    key_int = int.from_bytes(key_bytes, "big")
    
    print("--- Cifrado DES (Modo ECB Puro) ---")
    print(f"Texto original : {message_text}")
    print(f"Clave (8 bytes): {key_text}\n")
    
    # 3. Cifrar los datos
    cipher_bytes = des_encrypt(message_bytes, key_int)
    ...
    if cipher_base64 == "obuzqeTZFwc=":
        print("[EXITO] El cifrado coincide con el resultado esperado.")
    
    print("\n--- Descifrado DES ---")
    
    # 5. Descifrar partiendo del Base64
    # Decodificamos el Base64 de vuelta a los bytes crudos cifrados
    bytes_a_descifrar = base64.b64decode(cipher_base64)
    
    # Aplicamos la funcion de descifrado (que usa las claves en reversa)
    decrypted_bytes = des_decrypt(bytes_a_descifrar, key_int)
    
    # Decodificamos los bytes recuperados a texto plano
